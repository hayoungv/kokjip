...
| 다룬다 | 다루지 않는다 |
|---|---|
| 제품 요구사항 · 시스템 요구사항 · 기술 설계 · 구현 코드 | 시장 조사 · 경쟁 실사 · 고객 인터뷰 |
...
npm run db:migrate            # 데이터베이스에 표를 만든다
npm run dev                   # 개발 서버
```
...
| 명령 | 하는 일 |
|---|---|
| `npm run build` | 배포용으로 빌드하고 타입을 검사한다 |
| `npm test` | 단위 시험을 돌린다 |
| `npm run db:studio` | 데이터베이스 내용을 화면으로 본다 |
