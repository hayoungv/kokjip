...
| S4 | 전사 방식과 원가 상한 | 기기 안에서 녹음 단위로 전사. 원가 상한 1인 월 1,000원 | `AGENTS.md` §6-5 |
...
| S12 | 데모에 쓰는 판정 결과 | 실측 53건을 미리 넣고, 판정 연결은 열쇠가 있을 때만 | `AGENTS.md` §10 |
...
| 항목 | 내용 | 반영 위치 |
|---|---|---|
| 강도 | 강조한 말마다 1에서 3 사이의 강도가 붙고, 복습 목록은 강도 높은 순이다 | `AGENTS.md` §6-14 |
...
```
RESOLVED: 8 / DROPPED: 0 / TOTAL: 8
```

| 번호 | 토픽 | 결정 | 반영 위치 |
|---|---|---|---|
| L1 | 학습 자료가 제품에서 하는 역할 | 목차 항목이 복습의 단위, 강조한 말은 근거 | `AGENTS.md` §8-1 |
| L2 | 학습 자료를 누가 올리는가 | 강사가 녹음 허용에 동의할 때 함께 올린다 | `AGENTS.md` §8-2 |
| L3 | 학습 자료에서 무엇을 뽑는가 | 목차만 뽑고 강사가 확인. 형식은 넓게 받는다 | `AGENTS.md` §8-3 |
| L4 | 강조한 말과 목차를 잇는 방법 | 두 단계 — 회차 구간을 먼저 정하고 그 안에서 붙인다 | `AGENTS.md` §8-4 |
| L5 | 복습 목록 화면이 어떻게 바뀌는가 | 목차 항목을 나열, 순서는 강도 합계 | `AGENTS.md` §8-5 |
| L6 | 학습 자료가 없을 때 | 말을 강도 높은 순으로 나열. 목차를 기계가 만들지 않는다 | `AGENTS.md` §8-6 |
| L7 | 학습 자료의 보관과 권리 | 목차를 뽑고 나면 원본을 즉시 삭제, 목차만 6개월 | `AGENTS.md` §8-7 |
| L8 | 용어 정리 | 강의 기록·강조한 말·녹음. 화면에서는 시각 범위 | `AGENTS.md` §8-8 |
